...
public class AdminUserServlet extends HttpServlet {
    ...
    private void saveEmployee(HttpServletRequest request, String action) {
        ...
        if (isBlank(fullName) || isBlank(email) || branchId <= 0) {
            setFlash(request, "errorMessage", "Please enter full name, email and branch.");
            return;
        }
        ...
        employee.setRoleID(isUpdate ? roleId : 2); // Admin creates Owners only
        employee.setStatus(isBlank(status) ? "ACTIVE" : status.toUpperCase());

        boolean success;

        if (isUpdate) {
            success = adminUserDao.updateOwner(employee);
        } else {
    ...
    private void resetPassword(HttpServletRequest request) {
        ...
        boolean success = adminUserDao.resetOwnerPassword(employeeId, hashedPassword);
