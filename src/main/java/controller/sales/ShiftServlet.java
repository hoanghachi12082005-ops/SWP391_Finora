...
public class ShiftServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (activeShift != null) {
            ...
            if (sizeStr != null) {
                try { sizeValue = Integer.parseInt(sizeStr); if (sizeValue < 1) sizeValue = 10; }
                catch (NumberFormatException ignored) {}
            }

            int totalRecords = shiftDao.countShiftHistory(branchId);
            int pageSize = (sizeValue == 100 && totalRecords > 0) ? totalRecords : sizeValue;
            int totalPages = (int) Math.ceil((double) totalRecords / pageSize);
            if (totalPages < 1) totalPages = 1;
            if (page > totalPages) page = totalPages;

            int offset = (page - 1) * pageSize;
            List<Shift> history = shiftDao.getShiftHistoryPaginated(branchId, offset, pageSize);
