...
<div class="table-footer" style="width: 100%; display: flex; justify-content: space-between; align-items: center;">
    <div class="pagination-info">
        <form method="get" action="${baseUrl}">
            <c:if test="${not empty param.queryString}">
                <%
                    String qs = request.getParameter("queryString");
                    if (qs != null) {
                        String s = qs.startsWith("&") ? qs.substring(1) : qs;
                        if (!s.isEmpty()) {
            ...
            <c:choose>
                <c:when test="${totalPages <= 5}">
                    <c:forEach begin="1" end="${totalPages}" var="i">
                        <a href="${baseUrl}?page=${i}&sizeValue=${sizeValue}${param.queryString}"
                           class="${i == currentPage ? 'active-page' : ''}">${i}</a>
                    </c:forEach>
                </c:when>
                <c:otherwise>
                    <a href="${baseUrl}?page=1&sizeValue=${sizeValue}${param.queryString}"
                    ...
                    <a href="${baseUrl}?page=${totalPages}&sizeValue=${sizeValue}${param.queryString}"
