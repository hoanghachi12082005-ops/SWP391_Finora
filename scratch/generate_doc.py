...
def create_document():
    ...
    headers = ["No.", "File / Deliverable Name", "Notes & Description"]
    ...
    col_widths = [Inches(0.6), Inches(2.2), Inches(3.7)]
    ...
    add_p("This workflow governs product catalog setup, supplier relations, purchase orders, and goods receiving, ensuring accurate inventory levels across store warehouses.")

    add_custom_heading("3.1 Product Catalog & Category Management", level=3)
    add_p("Warehouse staff and Managers create and manage products via /product. Each product includes Name, Barcode, Category, Unit of Measurement, Purchase Price, Selling Price, Min Stock Alert Level, and image attachments.")
    ...
    add_custom_heading("3.2 Supplier Management & Purchase Orders", level=3)
    add_p("Staff maintain supplier contact directories (/supplier) and generate Purchase Orders (PO) for incoming stock intake:")
    add_p("Maintain supplier profiles, tax codes, contact details, and supplier product mappings.", bold_prefix="• Supplier Directory: ", bullet=True)
    add_p("Select supplier, destination warehouse, item quantities, and agreed purchase price. Upon goods receipt, the system automatically increments inventory quantities in the database and logs stock transactions.", bold_prefix="• Purchase Orders & Goods Import: ", bullet=True)
    ...
    add_img("scratch/images_rds/p_957_rId72.png", caption="Supplier Product Mapping Management (RDS)")
    add_img("scratch/images_rds/p_761_rId60.png", caption="Stock Import & Excel Bulk Intake Interface (RDS)")
    add_img("scratch/images_rds/p_762_rId61.png", caption="Stock Intake Confirmation & Batch Processing (RDS)")
    add_img("scratch/images_sds/p_163_rId11.png", caption="Direct Goods Importation Class Diagram (SDS)")
    ...
    # CORRECT PLACEMENT FOR 5.1: Inventory Check Screenshots & Class Diagram
    ...
    # CORRECT PLACEMENT FOR 5.2: Inter-Branch Stock Transfer Screenshots & Class Diagram
    ...
    # CORRECT PLACEMENT FOR 5.3: Stock History Screenshot & Class Diagram
    ...
    # Save document
    out_path = 'docs/Template Final (1).docx'
    try:
        doc.save(out_path)
        print(f"Document successfully created and saved at {out_path}")
    except PermissionError:
        alt_path = 'docs/Template Final (1)_updated.docx'
        doc.save(alt_path)
        print(f"File {out_path} is locked/open. Saved as {alt_path}")
