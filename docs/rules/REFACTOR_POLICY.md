...
5. Preserve UTF-8 without BOM when mass-editing files.
6. Run targeted verification.
7. Update docs if boundaries, patterns, build flow, or workflows changed.
...
Add real service logic only when business logic spans multiple DAOs, needs transaction boundaries, or is reused by page controllers and future API actions.

Do not add service logic for one controller calling one DAO.
...
- Do not hand-edit generated `build/` or `dist/` output.
